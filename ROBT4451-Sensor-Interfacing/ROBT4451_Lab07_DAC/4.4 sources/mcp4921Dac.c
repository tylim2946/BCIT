...
************************************************************************************/
char dacSet(double voltage)
{
    if (voltage >= 0 && voltage <= V_FSV)
    {
        volatile unsigned int dacWord = 4096 * voltage / V_FSV;

        dacWriteWord(dacWord, DAC_CTRL_SET_DEFAULT);
        return -1;
    }
    else
    {
...
void dacSetBin (unsigned int dacWord)
{
    dacWriteWord(dacWord, DAC_CTRL_SET_DEFAULT);
}

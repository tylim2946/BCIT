...
#define PWM_MIN 16 // when TA1CCR0 is 65535
#define PWM_MAX 116444 // when TA1CCR0 is 9
#define DC_MIN 0
#define DC_MAX 10

#define SMCLK_CLK 20E6
...
char timerA1Init(unsigned int pwmFreq) // 20kHz PWM Signal
{
    ...
    TA1CTL |= TASSEL_2 + MC_1 + ID_0; //up mode, divide by 1
...
char timerA1PwmFreqSet(unsigned int pwmFreq)
{
    ...
    if (pwmFreq >= PWM_MIN && pwmFreq <= PWM_MAX) // if the requested pwmFreq is outside the range, then TA1CCR0 is not changed and -1 is returned
    {
        TA1CCR0 = 1.0 / pwmFreq * SMCLK_CLK - 1 + 0.5; // compute and set TA1CCR0 value
    }
    else
    {
        output = -1;
...
char timerA1DutyCycleSet(unsigned char dutyCycle)
{
    ...
    {
        TA1CCR1 = (1.0 + TA1CCR0) * dutyCycle / 10; // compute and set TA1CCR1
    }
    else
    {
        output = -1;
...
void timerA1VoltageSet(double voltage)
{
    ...
    TA1CCR1 = (1.0 + TA1CCR0) * voltage / Vin + 0.5; // compute and set TA1CCR1
}
